...
template '/usr/local/etc/sensu/conf.d/check-bitlbee.json' do
  source 'check-bitlbee.json.erb'
  owner 'root'
  group 'wheel'
  mode 0555
  notifies :restart, "service[sensu_client]"
...
template '/usr/local/etc/sensu/conf.d/check-znc.json' do
  source 'check-znc.json.erb'
  owner 'root'
  group 'wheel'
  mode 0555
  notifies :restart, "service[sensu_client]"
